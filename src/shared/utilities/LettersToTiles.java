...
public class LettersToTiles {
    ...
    public static HashMap<String, String> tilesMap = new HashMap<String, Class>();

    public LettersToTiles() {
        tilesMap.put("N", "NullTile);
        tilesMap.put("A", "AirTile");
        tilesMap.put("W", "WaterTile");
        tilesMap.put("S", "SandTile");
        tilesMap.put("G", "GrassTile");
        tilesMap.put("C", "CrateTile");
    }
        ...
        for (int z1 = 0; z1 < letterMap.length; z1++) {
            for (int y1 = 0; y1 < letterMap[0].length; y1++) {
                for (int x1 = 0; x1 < letterMap[0][0].length; x1++) {
                    try {
						tiles[z1][y1][x1] = Tile.getTile(x + x1, y + y1, z + z1, 0, "default", tileMaps.get(letterMap[z1][y1][x1]);
